...
for q1 in train['question1'].values:
    q1 = (" ").join(list(process_new_string(q1)))
    train_1.append(q1)
for q2 in train['question2'].values:
    q2 = (" ").join(list(process_new_string(q2)))
    train_2.append(q2)
for q1 in eval_data['question1'].values:
    q1 = (" ").join(list(process_new_string(q1)))
    eval_1.append(q1)
for q2 in eval_data['question2'].values:
    q2 = (" ").join(list(process_new_string(q2)))
    eval_2.append(q2)

train_1 = pd.DataFrame(train_1)
train_2 = pd.DataFrame(train_2)
eval_1 = pd.DataFrame(eval_1)
eval_2 = pd.DataFrame(eval_2)
print("###############################")
print(train1.shape)
print(train2.shape)
train = pd.concat([train_1,train_2],axis=1)
print(train.shape)
train.to_csv("features/train_clean",index=False)

test = pd.concat([eval_1,eval_2],axis=1)
print(train.shape)
test.to_csv("features/test_clean",index=False)